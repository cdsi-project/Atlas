...
- 绑定多个目标的项目在同步时会依次备份到所有已勾选目标，并分别保存上传审计；项目列表显示目标数量、提供商和创建时间，项目资产列表显示短资源 ID、加入时间和备份状态，两张列表的每一列均可独立调整宽度
- 项目列表右键支持同步到云端和删除项目；删除只清理本地项目及成员关系并记录审计，不删除资产文件或已有云端备份
