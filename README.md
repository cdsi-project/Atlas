...
- “项目管理”页签可创建视频、音频、图片、文字或综合类型的项目；新建时可选择暂不绑定，或绑定已配置且凭据有效的阿里云 OSS、腾讯云 COS、七牛云 Kodo 备份目标，项目同步将固定使用绑定目标
- 项目列表显示云端备份目标和创建时间，项目资产列表显示短资源 ID、加入时间和备份状态，两张列表的每一列均可独立调整宽度
