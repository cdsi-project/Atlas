...
当前版本为 **v0.200**。仓库已经形成可运行的 Windows 桌面闭环：受管工作目录、多扫描目录、项目化资产管理、阿里云 OSS / 腾讯云 COS / 七牛云 Kodo 多目标备份与校验、云端取回、重复文件检测、基础媒体元数据、OpenWeb 文章发布、本地数据库冗余和运行日志。
...
- 绑定多个目标的项目在同步时会依次备份到所有已勾选目标，并分别保存上传审计；项目列表显示目标数量、提供商和创建时间并支持 <code>Ctrl/Shift</code> 多选，项目资产列表显示短资源 ID、加入时间和备份状态，两张列表的每一列均可独立调整宽度
- 项目列表右键支持同步到云端和单选或多选删除项目；删除只清理本地项目及成员关系并记录审计，不删除资产文件或已有云端备份。项目内资产可单选或多选右键“移出项目”，只移除成员关系，不删除文件、资产索引或云端备份
...
- 主界面提供“云备份管理”页签，按项目组织 Beacon 已登记的阿里云 OSS、七牛云 Kodo 和腾讯云 COS 对象；左侧显示项目的云提供商、项目资源数、副本数、占用空间和最近备份时间，右侧显示所选项目的云端备份，无项目关系的历史备份归入“未归属项目”。可按本地文件名、云端文件名、对象键、项目、备份配置或提供商搜索；双击备份可打开当前可用的本地文件位置，右键可单选或多选取回、删除所选云端备份，也可在左侧右键删除整个备份项目
...
Beacon 当前以“项目”作为云端操作边界：资产可以先被索引而暂时不属于项目，但只有加入项目后才能同步到云端。项目删除、项目成员移除、云端备份删除和本地文件删除是相互独立的操作，任何一项都不会隐式触发其他项。

当前 v0.200 的本地项目已经拥有稳定项目 ID，但云端对象仍使用 <code>&lt;项目名称&gt;/&lt;原文件名&gt;</code> 作为对象键，并由“云备份管理”按对象键前缀分组。这是兼容阶段的实现，不应把项目名称视为永久身份：同一备份配置下的同名项目可能发生对象键冲突，也不能仅凭目录名可靠区分来自不同设备的逻辑项目。
...
- 本机 SQLite 位于 <code>%LOCALAPPDATA%\CDSI\cdsi.db</code>。其中包含文件名、绝对路径、大小、时间、SHA-256、媒体元数据、项目及成员关系、远端对象键、操作审计和发布映射等隐私信息；能够访问该数据库的用户可能据此了解本机资产结构。
...
- 当前扫描流程不提取或保存文件正文。
- 当前版本不会因为扫描、搜索、创建项目或保存配置而传输文件。保存 Git 配置不会连接、克隆、提交或推送仓库；网络传输或云端变更只在用户明确执行云端备份、取回、删除或 OpenWeb 发布时发生。发布过程中可能按 Markdown Front Matter 创建所声明但尚不存在的 WordPress 分类或标签。
...
- <code>CDSI.Agent.Infrastructure</code>：文件系统扫描、媒体元数据提取、SQLite、Windows 凭据管理器，以及阿里云 OSS、腾讯云 COS、七牛云 Kodo 适配器。
