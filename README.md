...
- “项目管理”页签可创建视频、音频、图片、文字或综合类型的项目；项目列表显示创建时间，项目资产列表显示短资源 ID、加入时间和备份状态，两张列表的每一列均可独立调整宽度
- 项目列表右键支持同步到 OSS 和删除项目；删除只清理本地项目及成员关系并记录审计，不删除资产文件或已有 OSS 备份
- 从资产列表右键“加入项目”可直接选择最近的 3 个项目；项目超过 3 个时通过“更多...”打开完整选择，同一资产可属于多个项目
- 新的 OSS 备份必须以项目为单位：资产列表中的“同步到 OSS”只提供所选资产共同所属的项目；没有共同项目时，可先加入已有项目或新建项目再同步
- 将项目内全部可用资产同步到 OSS 的“项目名称/”目录，文件名保持本地原名，并执行上传进度和完整性校验
...
- 资产列表的“备份状态”和“备份时间”紧邻“所属项目”显示；默认状态为“未备份”，存在健康副本时按提供商显示“OSS”“七牛”“S3”等并使用绿色字体，同一资产的多个提供商合并显示；选中资产后，下方资产详情也显示相同备份信息
