...
- “扩展名”：从本机已经索引的扩展名中选择，例如 <code>.mp4</code>、<code>.jpg</code>、<code>.md</code>
- “创建时间”：开始日期和结束日期可分别启用，结束日期包含所选当天

选择条件后点击“搜索”。多个条件之间按 AND 组合，例如“视频 + <code>.mp4</code>”只显示 MP4 视频。搜索由 SQLite 在分页前执行，因此结果覆盖全部索引资产，而不只是当前页。点击“重置”可清空全部条件并回到完整资产列表。
